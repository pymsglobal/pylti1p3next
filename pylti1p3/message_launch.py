...
class MessageLaunch(t.Generic[REQ, TCONF, SES, COOK]):
    ...
    def __init__(
        ...
        service_connector_cls: t.Optional[type[ServiceConnector]] = ServiceConnector,
    ):
    ...
    def get_iss(self) -> str:
        ...
        if not iss:
            raise LtiMessageValidationException('The "iss" field in the launch message is empty.')
        return iss
    ...
    def _get_iss(self) -> str:
        iss = self._get_jwt_body().get("iss")
        if not iss:
            raise LtiMessageValidationException('The LTI launch message "iss" field is empty.')
        return iss

    def _get_id_token(self) -> str:
        id_token = self._get_request_param("id_token")
        if not id_token:
            raise LtiMessageValidationException('The LTI launch message "id_token" field is empty.')
        return id_token
    ...
    def _get_deployment_id(self) -> str:
        ...
        if not deployment_id:
            raise LtiMessageValidationException('The "deployment_id" field is not set in the launch message\'s JWT body.')
    ...
    def get_nrps(self) -> NamesRolesProvisioningService:
        ...
        if not names_role_service:
            raise LtiJWTException('The "namesroleservice" field is not set in the launch message\'s JWT body.')
    ...
    def get_cgs(self) -> CourseGroupsService:
        ...
        if not context_groups_url:
            raise LtiJWTException('The "context_groups_url" field is not set in the groupsservice section.')
    ...
    def fetch_public_key(self, key_set_url: str) -> TKeySet:
        ...
        with DisableSessionId(self._public_key_cache_data_storage):
            ...
            try:
                ...
                raise LtiKeyException(
                    f"Invalid response while fetching the platform's public key from {key_set_url}. Must be JSON: {resp.text}"
                ) from e
    ...
    def get_public_key(self) -> t.Tuple[str, str]:
        ...
        if not public_key_set:
            ...
            if key_set_url.startswith(("http://", "https://")):
                ...
                raise LtiKeyException("The public key set URL is invalid: " + key_set_url)
    ...
    def validate_jwt_format(self) -> "MessageLaunch":
        ...
        if len(jwt_parts) != 3:
            ...
            raise LtiMessageValidationException("Invalid id_token, JWT must contain 3 parts")
        ...
        try:
            ...
            raise LtiMessageValidationException("Invalid JWT format, can't be decoded") from e
    ...
    def validate_registration(self) -> "MessageLaunch":
        ...
        if client_id != self._registration.get_client_id():
            raise LtiMessageValidationException("Client id not registered for this issuer")
    ...
    def validate_jwt_signature(self) -> "MessageLaunch":
        ...
        try:
            ...
            raise LtiMessageValidationException(f"Can't decode id_token: {str(e)}") from e
    ...
    def validate_deployment(self) -> "MessageLaunch":
        ...
        if not deployment:
            raise LtiMessageValidationException('The deployment ID "{deployment_id}" is not recognised.'.format(deployment_id=deployment_id))
